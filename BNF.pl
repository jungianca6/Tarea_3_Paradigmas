...
% Estructura de oraciones que reciben negaciones o afirmaciones
oracion(S0, S) :- negacion(S0, S).  % Solo negación
oracion(S0, S) :- afirmacion(S0, S).  % Solo afirmación

% Negación seguida de un sintagma verbal
oracion(S0, S) :- negacion(S0, S1), sintagma_verbal(S1, S).

% Afirmación seguida de un sintagma verbal
oracion(S0, S) :- afirmacion(S0, S1), sintagma_verbal(S1, S).

% Negación seguida de un sintagma nominal
oracion(S0, S) :- negacion(S0, S1), sintagma_nominal(S1, S).

% Afirmación seguida de un sintagma nominal
oracion(S0, S) :- afirmacion(S0, S1), sintagma_nominal(S1, S).
...
% Manejo de negaciones en los sintagmas verbales
sintagma_verbal(S0, S) :- verbo(S0, S1), negacion(S1, S).  % Verbo seguido de negación
sintagma_verbal(S0, S) :- verbo(S0, S1), sintagma_nominal(S1, S).  % Verbo seguido de sintagma nominal
...
%###################Definiciones para afirmaciones###################
% Definiciones para afirmaciones
afirmacion([si | S], S).
afirmacion([afirmativo | S], S).
afirmacion([correcto | S], S).
afirmacion([claro | S], S).
afirmacion([cierto | S], S).
afirmacion([por supuesto | S], S).
afirmacion([exacto | S], S).
afirmacion([definitivamente | S], S).
afirmacion([así | S], S).
afirmacion([sin duda | S], S).
afirmacion([eso | S], S).
afirmacion([totalmente | S], S).

% Definiciones para las negaciones
negacion([no | S], S).
negacion([negativo | S], S).
negacion([nel | S], S).
negacion([jamás | S], S).
negacion([nunca | S], S).
negacion([en absoluto | S], S).
negacion([no quiero | S], S).
negacion([no puedo | S], S).
